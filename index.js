...
function apiCall() {
  fetch("https://different-northern-exhaust.glitch.me/movies")
    .then((data) => data.json())
    .then((res) => {
      console.log(res);
      let uTag = document.getElementById("movie");
      for (let i = 0; i < res.length; i++) {
        let lTag = document.createElement("li");
        let imgTag = document.createElement("img");
        let spanTag = document.createElement("span");
        spanTag.textContent = res[i].title + " Rating: " + res[i].rating;

        imgTag.src = res[i].poster;
        lTag.appendChild(imgTag);
        lTag.appendChild(spanTag);
        uTag.appendChild(lTag);
      }
    });
}
